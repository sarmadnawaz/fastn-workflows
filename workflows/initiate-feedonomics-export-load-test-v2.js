export default async function (ctx) {
  ...
  const startCursor = input.startCursor ?? 0; // keyset cursor (last product id), replaces OFFSET
  ...
  // VALIDATION HOOK (v2 only): inject a resume state to exercise the resume math
  // without an infra interruption. Removed from the production version.
  if (!resume && input.__resumeState) resume = input.__resumeState;
  ...
  try {
    ...
    const uploadRes = await fastn.connector.sftpServer.uploadFile({
      path: remotePath,
      file: { fileId: `fdx-job-${job.id}`, key: fileKey, bucket: 'fastn-files', size: 0, mime: 'text/tab-separated-values', name: finalName },
    });
